...
function autoload($aClassName) {
  $classFile = "/src/{$aClassName}/{$aClassName}.php";
   $file1 = MINI_INSTALL_PATH . $classFile;
   $file2 = MINI_SITE_PATH . $classFile;
   if(is_file($file1)) {
      require_once($file1);
...
function exceptionHandler($e) {
  echo "Mini: Uncaught exception: <p>" . $e->getMessage() . "</p><pre>" . $e->getTraceAsString(), "</pre>";
}
...
function htmlent($str, $flags = ENT_COMPAT) {
  return htmlentities($str, $flags, CMini::Instance()->config['character_encoding']);
}
